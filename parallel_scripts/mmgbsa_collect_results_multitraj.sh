...
# Or even simpler (if we rely also on the right cat * order):
...
do
        job_num_formatted=`printf %04i $counter`
        cp $current/$job_num_formatted/total/sasa.dat ./tmp/sasa-${job_num_formatted}.dat
        sed -i 's/[[:space:]]\{1,\}/,/g' ./tmp/sasa-${job_num_formatted}.dat
        sed -i 's/^,//g' ./tmp/sasa-${job_num_formatted}.dat
        counter=$((counter+1))
done

unset counter job_num_formatted

cat ./tmp/* > ./total/sasa.dat

echo -e "buried <- read.csv('./total/sasa.dat', header=FALSE)\n\
\n\
i <- seq(1, nrow(buried), by=1)\n\
buried <- buried[,2:6]\n\
buried <- cbind(i, buried)\n\
write.table(buried, './total/sasa.dat', row.names=FALSE, col.names=FALSE, sep=',', quote=FALSE)\n\
q(save='no')" > ./tmp.R

Rscript tmp.R
rm tmp.R

sed -i 's/^/ /' ./total/sasa.dat
sed -i 's/,/  /' ./total/sasa.dat
sed -i 's/,/  /' ./total/sasa.dat
sed -i 's/,/  /' ./total/sasa.dat
sed -i 's/,/ /' ./total/sasa.dat
...
do
        job_num_formatted=`printf %04i $counter`
        cp $current/$job_num_formatted/total/solv-global.dat ./tmp/solv-global-${job_num_formatted}.dat
        sed -i 's/[[:space:]]\{1,\}/,/g' ./tmp/solv-global-${job_num_formatted}.dat
        sed -i 's/^,//g' ./tmp/solv-global-${job_num_formatted}.dat
        counter=$((counter+1))
done

unset counter job_num_formatted

cat $current/tmp/* > ./total/solv-global.dat

echo -e "solv_a_global <- read.csv('./total/solv-global.dat', header=FALSE)\n\
\n\
i <- seq(1, nrow(solv_a_global), by=1)\n\
solv_a_global <- solv_a_global[,2]\n\
solv_a_global <- cbind(i, solv_a_global)\n\
write.table(solv_a_global, './total/solv-global.dat', row.names=FALSE, col.names=FALSE, sep=',')\n\
q(save='no')" > ./tmp.R

Rscript tmp.R
rm tmp.R

sed -i 's/^/ /' ./total/solv-global.dat
sed -i 's/,/ /' ./total/solv-global.dat
